...
def get_cf_speed_test_ip_dx(timeout=30, max_retries=3):
    """
    直接从 https://ip.164746.xyz/ipTop10.html 接口获取优选IP
    """
    ...
    for attempt in range(max_retries):
        try:
            ...
            if response.status_code == 200:
                ...
                # 使用内置字符串方法替代 re 模块进行分割
                raw_ips = text.replace('\n', ',').replace(' ', ',').split(',')
                
                # 使用简单的内置方法过滤出有效的 IPv4 (包含点，且不含冒号)
                telecom_ips = [ip.strip() for ip in raw_ips if ip.strip() and '.' in ip and ':' not in ip]
                ...
                if telecom_ips:
                    ...
                    print("接口返回内容中未发现有效的 IPv4 优选IP")
                    print(f"接口原始返回: {text[:200]}")
            else:
                print(f"获取失败，HTTP 状态码: {response.status_code}")

        except Exception as e:
            # 去掉 traceback，直接打印异常信息
    ...
    for attempt in range(max_retries):
        try:
            ...
            if response.status_code == 200:
                ...
                return [record['id'] for record in records if record['name'] == name]
            else:
...
def main():
    ...
    if ',' in CF_DNS_NAME:
        ...
        for index, dns_name in enumerate(dns_names):
            ...
            update_dns_record(dns_records[0], dns_name, ip_addresses[index])
            time.sleep(2)
