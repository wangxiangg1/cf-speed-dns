...
def get_cf_speed_test_ip_dx(timeout=30, max_retries=3):
    """
    直接从 https://ip.164746.xyz/ipTop10.html 接口获取优选IP（Top10）
    返回内容预期为逗号分隔的 IP 列表
    """
    ...
    for attempt in range(max_retries):
        try:
            ...
            if response.status_code == 200:
                ...
                # 兼容多种分隔符：逗号 / 换行 / 空白
                raw_ips = re.split(r'[,\s]+', text)
                # 只保留有效的 IPv4，过滤掉 IPv6 和空字符串
                ipv4_pattern = re.compile(r'^(\d{1,3}\.){3}\d{1,3}$')
                telecom_ips = [ip.strip() for ip in raw_ips if ip.strip() and ipv4_pattern.match(ip.strip())]
